...
var (
	// ContextMeter is the meter used for recording context control metrics.
	ContextMeter metric.Meter = MeterProvider.Meter(metrics.MeterNameContext)

	// Context histogram metrics.
...
type ContextConfigSnapshot struct {
	...
	AddSummary                   bool
	TailoringStrategy            string // "middle_out", "head_out", "tail_out", "none"
...
type ContextMetricsTracker struct {
	...
	// Initial state (after preprocess, before any control).
	initialMessageCount int
	initialTokens       int

	// 1. History trim (max_history_runs).
	...
	// 3. Context compaction (context_compaction_threshold_ratio).
	compactionTriggered  bool
	preCompactionTokens  int
	postCompactionTokens int

	// 4. Tool result compaction (tool_result_max_tokens).
	toolCompactionTriggered   bool
	toolCompactionTokensSaved int

	// 5. Oversized tool result truncation (oversized_tool_result_max_tokens).
